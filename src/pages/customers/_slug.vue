  ...
  setup() {
    ...
    const setCustomer = (data) => {
      let customer = {
        ...
        contactName: data.contact_given + " " + data.contact_family,
        email: data.email,
        phoneNUmber: data.home_phone,
        mobileNumber: data.mobile_phone,
      };
      items.value = customer;
    };

    const fetchCustomer = async (accountUCN) => {
      await api.get("sanctum/csrf-cookie");
      try {
        const data = await api.post("user/get-account-customer", {
          ucn: route.params,
        });
