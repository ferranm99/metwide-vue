        ...
        <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
          ...
          <div class="mb-8">
            ...
            <h1 class="text-2xl md:text-3xl text-slate-800 font-bold">
             Subscription#: {{ $route.params.slug }}
            </h1>
          </div>
  ...
  setup() {
    ...
    const setSubscription = (data) => {
      let subscription = {
        ...
        "Service Status": data.billing_status,
        "Service Type": data.smile_service_name,
        "Plan": data.smile_plan ? data.smile_plan.name : '',
        "Description": data.description,
      };
      items.value = subscription;
    };

    const fetchSubscription = async (accountUCN) => {
      await api.get("sanctum/csrf-cookie");
      try {
        const data = await api.post("user/get-subscription", {
          usn: route.params,
        });
