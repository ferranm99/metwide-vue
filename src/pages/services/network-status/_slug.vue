        ...
        <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
          ...
          <div class="mb-8">
            ...
            <h1 class="text-2xl md:text-3xl text-slate-800 font-bold">
              Network Status Event ID: {{ $route.params.slug }}
            </h1>
          </div>
...
export default {
  ...
  setup() {
    ...
    const setNetworkStatus = (data) => {
      let networkStatus = {
        "Event ID": data.event_id,
        username: data.vocus_service.username,
        Status: data.status,
        Description: data.description,
        "Start Date Time": moment(data.start_date_time).format(
          "DD-MMM-YYYY HH:mm:ss"
        ),
        "End Date Time": moment(data.end_date_time).format(
          "DD-MMM-YYYY HH:mm:ss"
        ),
        Duration: data.duration,
        "Created At": moment(data.created_at).format("DD-MMM-YYYY HH:mm:ss"),
      };
      items.value = networkStatus;
    };

    const fetchNetworkStatus = async (accountUCN) => {
      await api.get("sanctum/csrf-cookie");
      try {
        const data = await api.post("user/get-network-status", {
          eventID: route.params,
        });
