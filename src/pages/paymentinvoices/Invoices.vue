  ...
  <div>
    ...
    <section class="bg-white shadow-lg rounded-sm mb-8 p-6 space-y-6">
      ...
      <table class="table-auto w-full">
        <!-- Table header -->
        <thead
          class="text-xs uppercase text-slate-400 border-b border-primary-300"
        >
          <tr class="flex flex-wrap md:table-row md:flex-no-wrap">
            ...
            <th class="w-full block md:w-auto md:table-cell py-2">
              <div class="font-semibold text-left">Invoice Date</div>
            </th>
        ...
        <tbody class="text-sm">
          ...
          >
            ...
            <td class="w-full block md:w-auto md:table-cell py-0.5 md:py-2">
              <div class="text-left">{{ invoice.dueDate }}</div>
            </td>
            <td class="w-full block md:w-auto md:table-cell py-0.5 md:py-2">
              <div class="text-left font-medium">{{ invoice.invoiceNo }}</div>
            </td>
            <td class="w-full block md:w-auto md:table-cell py-0.5 md:py-2">
              <div class="text-left font-medium">
                {{ currencyWithCommas(invoice.amount) }}
              </div>
            </td>
            <td class="w-full block md:w-auto md:table-cell py-0.5 md:py-2">
  ...
  setup() {
    ...
    const setInvoices = (data) => {
      ...
      data.forEach((element) => {
        let invoice = {
          name: element.name,
          invoiceDate: moment(element.entry_date).format("DD-MMM-YYYY"),
          dueDate: moment(element.due_date).format("DD-MMM-YYYY"),
          invoiceNo: element.transaction_number,
    ...
    const filterInvoicesByDate = computed(() => {
      ...
      return invoices.value
        .filter((data) => {
          ...
          var date = moment(data.invoiceDate).format("YYYY-MM-DD");
          return date >= start && date <= end;
        })
        .sort((a, b) => new Date(b.invoiceDate) - new Date(a.invoiceDate));
    });

    const fetchInvoices = async (accountUCN) => {
      let ucn = accountUCN;
      await api.get("sanctum/csrf-cookie");
      try {
        const data = await api.post("user/get-invoices", {
          ucn: ucn,
        });
    ...
    fetchInvoices(user.$state.ucn);


    const filteredInvoices = computed(() => {
      let arr = selectFilter.value.map((el) => el.toLowerCase());
      if (search.value) {
        return filterInvoicesByDate.value.filter((item) => {
          return (
        ...
        for (let index = 0; index < selectFilter.value.length; index++) {
          if (selectFilter.value.includes("all")) {
            return filterInvoicesByDate.value;
